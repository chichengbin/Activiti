...
public class ActivitiRule implements TestRule {
  ...
  protected void starting(Description description) {
    ...
    try {
      AnnotationSupport.annotationMockSupportSetup(Class.forName(description.getClassName()), description.getMethodName(), mockSupport);
    } catch (ClassNotFoundException e) {
      throw new ActivitiException("Programmatic error: could not instantiate " + description.getClassName(), e);
    }

    try {
      deploymentId = AnnotationSupport.annotationDeploymentSetUp(processEngine, Class.forName(description.getClassName()), description.getMethodName());
    } catch (ClassNotFoundException e) {
      throw new ActivitiException("Programmatic error: could not instantiate " + description.getClassName(), e);
    }
  }
  ...
  protected void finished(Description description) {
    ...
    try {
      AnnotationSupport.annotationDeploymentTearDown(processEngine, deploymentId, Class.forName(description.getClassName()), description.getMethodName());
    } catch (ClassNotFoundException e) {
      throw new ActivitiException("Programmatic error: could not instantiate " + description.getClassName(), e);
    }
    ...
    // Rest mocks
    if (mockSupport != null) {
      AnnotationSupport.annotationMockSupportTeardown(mockSupport);
    }
  }
